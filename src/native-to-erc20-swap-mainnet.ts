...
async function main() {
  ...
  const INPUT_TOKEN = mantleMainnetTokens.wnative; // WMANTLE for native swaps
  const OUTPUT_TOKEN = mantleMainnetTokens.usdc;
  ...
  if (
    nativeBalance < parseUnits("0.1", mantleMainnet.nativeCurrency.decimals)
  ) {
