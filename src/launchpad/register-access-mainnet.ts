/**
 * Launchpad Access Registration Example (Mainnet)
...
async function main() {
  logger.header("🔐 Launchpad Access Registration (Mainnet)");
  ...
  if (!accessCode.trim()) {
    ...
    logger.info(
      "Example: ACCESS_CODE=your-invite-code npm run mainnet:launchpad:register-access"
    );
    process.exit(1);
  }
