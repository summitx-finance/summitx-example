...
async function main() {
  logger.header("⚡ Native MANTLE V3 Concentrated Liquidity");
...
async function addV3NativeLiquidity(
  ...
  logger.header("\n💧 Add V3 Native MANTLE Concentrated Liquidity");

  // Define native V3 liquidity token pairs (excluding WMANTLE since we're using native)
  const NATIVE_V3_PAIR_TOKENS = [mantleMainnetTokens.usdc];
  ...
  logger.success(
    `\n✅ Selected pair: ${isNativeToken0 ? "MANTLE" : selectedToken.symbol}/${
  ...
  if (!inRange) {
    if (currentTick < tickLower) {
      ...
      if (isNativeToken0) {
        ...
        logger.info("✅ Price is below your range. You'll only provide MANTLE");
        tokenAmount = 0n;
      }
    } else {
      // Above range: only need token0 (lower value token)
      if (isNativeToken0) {
        logger.info("✅ Price is above your range. You'll only provide MANTLE");
        tokenAmount = 0n;
      } else {
        logger.warn(
  ...
  logger.info("\n💧 Adding V3 concentrated liquidity with native MANTLE...");
...
async function removeV3NativeLiquidity(
  ...
  logger.header("\n💧 Remove V3 Native MANTLE Liquidity");
  ...
  // Display positions
  logger.success(
    `\n📊 Found ${nativePositions.length} native MANTLE V3 position(s):\n`
  );
