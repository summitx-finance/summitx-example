...
export class QuoteToTradeConverterV2 {
   ...
   */
  static convertQuoteToTrade(
    quote: QuoteResult
  ): SmartRouterTrade<CoreTradeType> {
  ...
  private static parseRoutes(
    ...
    const routeStrings = Array.isArray(quote.route)
      ? quote.route
      : [quote.route];
  ...
  private static buildPoolsAndPath(
    ...
    console.warn(
      "Building pools from string representation - this may not have accurate pool data"
    );
    ...
    if (routeInfo.hops.length === 0) {
      ...
      const poolType = poolInfoStr.includes("V3")
        ? PoolType.V3
        : poolInfoStr.includes("Stable")
        ? PoolType.STABLE
        : PoolType.V2;
      ...
      throw new Error(
        "Multi-hop swaps cannot be accurately converted without raw trade data. Please ensure rawTrade is included in the quote."
      );
    }
  ...
  private static determineRouteType(
    pools: Array<V2Pool | V3Pool | StablePool>
  ): RouteType {
  ...
  static validateConversion(
    quote: QuoteResult,
    trade: SmartRouterTrade<CoreTradeType>
  ): boolean {
    // Check amounts match
    const inputMatches =
    ...
    const inputCurrencyMatches = trade.inputAmount.currency.equals(
      quote.inputToken
    );
    const outputCurrencyMatches = trade.outputAmount.currency.equals(
      quote.outputToken
    );
    ...
    return (
      ...
      inputCurrencyMatches &&
      outputCurrencyMatches
    );
  }
}
